...
for (method in cor_methods) {
    test_that(paste("method", method, "is correct"), {
        expect_lte(
            abs(indep_test(x, y, method = method)$p_value -
                cor.test(x, y, method = method, exact = FALSE)$p.value),
            3e-2
        )
        expect_lte(
            abs(indep_test(xt, yt, method = method)$p_value -
...
test_that("method hoeffding is correct", {
    skip_if_not_installed("Hmisc")
    expect_lte(
        abs(indep_test(x, y, method = "hoeffd")$p_value -
...
test_that("method blomqvist is correct", {
    ...
    expect_lte(abs(mean(ps < 0.05) - 0.05), 0.025)
    expect_lte(abs(mean(ps < 0.01) - 0.01), 0.01)
})
